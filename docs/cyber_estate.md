...
| scan/ (local): the configured scope is too large for a flat liveness timeout | The backstop is derived from the address count of the configured targets, floored at 300s and capped at `DEFAULT_TIMEOUT` | A constant 300s could never finish a `/16`, which is an ordinary answer under *Configure → Networks to scan*, so that scope failed every time forever — a constant silently contradicting the configuration, which `scan/const.py`'s own header forbids |
